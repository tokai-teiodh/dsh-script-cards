...
- **画布**：`滚轮`缩放（以指针为中心，走 25/33/50/67/80/100/125/150/200% 这些档位）、
  ...
  从卡片边缘的圆点（整个圆、空心）拖出贝塞尔连线；底部是 iOS 后台式的堆叠条，
  把没上画的卡片拖进去或点一下落位。
- **四种卡片形态**
  ...
  - 分歧节点：在节点基础上，右侧一列**选项**，每个选项各带一个可拖的出口，卡片上就有
    「＋ 选项」；接通后标出走向（`→`），展开后选项在下面单列一块，带一键增删。
...
- 这个替身**没有布局引擎**，「元素被 CSS 裁掉了」这类问题它看不见。所以画布那几条
  关键样式是用**契约断言**钉住的：卡片不许 `overflow:hidden`（选项列挂在卡片盒子外面）、
  接口圆点必须是空心圆、画布层不许 `will-change`、`translate` 必须整像素。
  真机上真出过的两个「看不见」的 bug 都是这一层（选项列整列被裁、画布糊）。
...
测不到的部分（真 DOM 布局与 CSS 效果、SVG 箭头、真实 Typert 网关）要靠真机重启验证。
