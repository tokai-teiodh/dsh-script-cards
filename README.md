...
- **方片**：自适应圆角方片网格、按类型分组、标签单行横滑（滚动条平时一点都看不见，
  滑动时才浮出一根细条，也没有 Windows 那两个三角箭头）、星标收藏 + 置顶、点开看正文。
...
- **画布**：`滚轮`缩放（以指针为中心，走 25/33/50/67/80/100/125/150/200% 这些档位）、
  ...
  **入口会点亮**表示可以落，松手即连；**拖动卡片时连线实时跟着走**（不是松手才跳过去）；
  ...
  接着改；右键那条线还有重命名 / 删除的菜单。底部是 iOS 后台式的堆叠条，
  把没上画的卡片拖进去或点一下落位。
- **四种卡片形态**
...
- 这个替身**没有布局引擎**，也不跑 CSS，「元素被裁掉了 / 画到屏幕外了」这类问题它看不见。
  ...
  **标签条的滚动条整个藏掉**（`scrollbar-width:none`，滑动时才由 JS 浮出一根绝对定位的
  细条，不占位也就不会抖）、画布层不许 `will-change`、`translate` 必须整像素。
  真机上真出过的五个「看不见」的 bug 都是这一层（选项列整列被裁、画布糊、连线画到屏幕外、
  连线名字根本没画出来、对话框里那条多余的横向滚动条）。
